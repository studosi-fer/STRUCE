...
%disp('Centroidi obični:');
%disp(cent);

% Kod k-meansa nismo vršili normalizaciju, a kod EM-a jesmo. Zato sada moramo
% normalizirati ova nova središta koristeći vrijednosti dobivene tijekom
% normalizacije čitavog skupa primjera.
for k = 1:K,
  cent(k,:) = (cent(k,:).-middles')./(spans');
end

%disp('Centroidi normalizirani:');
%disp(cent);

disp('Centroidi before:');
disp(cent);
